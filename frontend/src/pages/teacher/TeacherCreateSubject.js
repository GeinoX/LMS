...
import { underControl, setActiveSubject } from '../../redux/userRelated/userSlice';
...
const TeacherCreateSubject = () => {
  ...
  const sclassName = currentUser?.teachSclass?._id;
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      formData.append('uploaderRole', 'teacher'); // Teacher is uploading subjects
      formData.append('uploaderId', teacherID); // Teacher ID
      ...
      const sendPromise = dispatch(addStuff(formData, `Teacher/${teacherID}/Subject`));
  ...
  useEffect(() => {
    ...
    switch (status) {
      case 'added':
        ...
        // Refresh teacher so new subject(s) appear immediately
        (async () => {
          try {
            const refreshed = await axios.get(`${process.env.REACT_APP_BASE_URL}/Teacher/${teacherID}`);
            if (refreshed.data) {
              // Update local storage user to keep state consistent across reloads
              localStorage.setItem('user', JSON.stringify(refreshed.data));
              // Set active subject to the most recently created one if possible, otherwise first
              const newSubjects = Array.isArray(refreshed.data.teachSubjects) ? refreshed.data.teachSubjects : [];
              const newActive = newSubjects.length > 0 ? String(newSubjects[newSubjects.length - 1]._id) : null;
              if (newActive) dispatch(setActiveSubject(newActive));
            }
          } catch (e) {
            console.warn('Failed to refresh teacher after subject creation:', e);
          } finally {
            dispatch(underControl());
            navigate('/Teacher/subject/files');
          }
        })();
        break;
  ...
  }, [status, response, error, navigate, dispatch, teacherID]);
